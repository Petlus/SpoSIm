...
const BracketMatch = ({ match, compact = false, twoLegged = false }: { match: any; compact?: boolean; twoLegged?: boolean }) => {
    ...
    return (
        <div className={`bg-slate-800/90 border border-slate-600/80 rounded-xl overflow-hidden shadow-lg hover:border-slate-500/60 transition-all ${compact ? 'w-40' : 'w-52'} min-w-0`}>
            {twoLegged && (
                <div className="px-2 py-0.5 bg-amber-500/20 text-amber-400 text-[9px] font-bold uppercase tracking-wider text-center border-b border-slate-700/80">
                    2 legs
                </div>
            )}
            <div className={`flex items-center justify-between px-3 py-2.5 gap-2 ${homeWon ? 'bg-emerald-500/15 border-l-2 border-emerald-500' : 'border-l-2 border-transparent'}`}>
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className="w-6 h-6 rounded-md bg-slate-700/80 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {match.home?.logo ? <img src={match.home.logo} className="w-5 h-5 object-contain" alt="" /> : <span className="text-[10px]">⚽</span>}
                    </div>
                    <span className={`truncate text-sm font-semibold ${homeWon ? 'text-emerald-300' : 'text-slate-200'}`}>
                        {match.home?.short_name || match.home?.name || 'TBD'}
                    </span>
                </div>
                <span className={`font-mono font-bold text-base tabular-nums ${homeWon ? 'text-emerald-400' : 'text-slate-400'}`}>
            ...
            <div className={`flex items-center justify-between px-3 py-2.5 gap-2 border-t border-slate-700/80 ${awayWon ? 'bg-emerald-500/15 border-l-2 border-emerald-500' : 'border-l-2 border-transparent'}`}>
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className="w-6 h-6 rounded-md bg-slate-700/80 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {match.away?.logo ? <img src={match.away.logo} className="w-5 h-5 object-contain" alt="" /> : <span className="text-[10px]">⚽</span>}
                    </div>
                    <span className={`truncate text-sm font-semibold ${awayWon ? 'text-emerald-300' : 'text-slate-200'}`}>
                        {match.away?.short_name || match.away?.name || 'TBD'}
                    </span>
                </div>
                <span className={`font-mono font-bold text-base tabular-nums ${awayWon ? 'text-emerald-400' : 'text-slate-400'}`}>
...
const RoundLabel = ({ children }: { children: React.ReactNode }) => (
    <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400/80 mb-3 text-center">
        {children}
    </div>
);

// Bracket column with matches
const BracketColumn = ({ label, matches, matchGap = 'gap-6', compact = false, twoLegged = false }: { label: string; matches: any[]; matchGap?: string; compact?: boolean; twoLegged?: boolean }) => (
    <div className="flex flex-col items-center">
        ...
        <div className={`flex flex-col items-center ${matchGap}`}>
            {matches.map((m: any) => (
                <BracketMatch key={m.id} match={m} compact={compact} twoLegged={twoLegged} />
            ))}
        </div>
    </div>
);
...
export default function TournamentPage() {
    ...
    return (
        <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen relative">
            ...
            {/* League Phase Tab - Full-width table */}
            {activeTab === 'league' && (
                                ...
                                return (
                                    <tr key={t.id} className={inTop24 ? 'bg-emerald-500/5' : ''}>
                                        ...
                                        <td className="font-semibold flex items-center gap-3">
                                            ...
                                            <Link href={`/football/team/${t.id}`} className="text-slate-100 truncate hover:text-emerald-400 hover:underline decoration-emerald-500/50 underline-offset-4 transition-all">
                                                {t.short_name || t.name}
                                            </Link>
                                        </td>
            ...
            {activeTab === 'fixtures' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
                        ...
                        <button
                            onClick={runSimulateMatchday}
                            disabled={simulating}
                            className="px-5 py-2.5 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 rounded-xl font-bold text-white text-sm shadow-lg shadow-amber-900/20 transition-all disabled:opacity-50 flex items-center gap-2 border border-amber-500/20"
                        >
                                ...
                                <div key={f.id ?? i} className="glass-card p-4">
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        {(f.homeScore !== null && f.awayScore !== null) ? (
                                            <span className="font-mono font-bold text-white mx-3">
                                    ...
                                    {(f.homeScore !== null && f.awayScore !== null) ? null : (
                                        <div className="space-y-2">
                                            ...
                                            <button
                                                onClick={() => simulateSingleMatch(i, f.id)}
                                                disabled={f.simulating || !f.id || f.homeScore !== null}
                                                className="w-full h-8 bg-amber-600/80 hover:bg-amber-500 rounded border border-amber-500/50 flex items-center justify-center gap-2 text-xs font-bold text-white transition-all disabled:opacity-50"
                                            >
            ...
            {activeTab === 'bracket' && bracket && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex justify-center mt-10">
                        <button
                            onClick={runSimulateRound}
                            disabled={simulatingRound}
                            className="px-8 py-3.5 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 rounded-xl font-bold text-white shadow-lg shadow-amber-900/40 transition-all flex items-center gap-2 border border-amber-500/30 disabled:opacity-50"
                        >
